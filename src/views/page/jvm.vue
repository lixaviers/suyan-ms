...
<h3><span style="font-size: 14pt;">二、JVM内存为什么要分成新生代、老年代、持久代，新生代中为什么要分为Eden和Survivor？</span></h3>
<h4>2.1 新生代</h4>
...
<h4>2.4 新生代中为什么要分为Eden和Survivor</h4>
<ul>
...
<h3><span style="font-size: 14pt;">三、JVM中一次完整的GC流程是怎样的，对象如何晋升老年代？</span></h3>
...
<h3><span style="font-size: 14pt;">四、你知道的垃圾收集器，各自的优缺点</span></h3>
<h4>4.1 Serial收集器</h4>
<div>最基本、历史最悠久的垃圾收集器。<br />它的单线程的意义不仅仅意味着它只会使用一条垃圾收集器线程去完成垃圾收集工作，更重要的是它在进行垃圾收集器工作的时候必须暂停其他所有的工作线程(Stop The World)，直到它收集结束<br />特点：</div>
<ol>
<li>针对新生代的收集器</li>
<li>采用复制算法</li>
<li>单线程收集</li>
...
<h4>4.3 Parallel Scavenge收集器</h4>
...
<h4>4.6 CMS(Concurrent Mark Sweep)收集器</h4>
<div><span style="background-color: #ced4d9;"><span style="background-color: #ffffff;">CMS(Concurrent Mark Sweep)收集器是一种以获取最短回收停顿时间为目标的收集器。它非常适合在注重用户体验的应用上使用。采用标记清除算法，运作过程：初始标记、并发标记、重新标记、并发清除，收集结束会产生大量空间碎片。CMS是HotSpot在JDK1.5推出的第一款真正意义上的并发收集器。</span></span></div>
...
<li>重新标记：就是为了修正并发标记期间因为用户程序继续运行而导致标记产生变动的那一部分对象的标记记录(采用多线程并行来提升效率)；需要Stop The World，且停顿时间比初始标记稍长，但远比并发标记短。</li>
...
<h3><span style="font-size: 14pt;">五、垃圾收集算法</span></h3>
...
<div>4、复制算法：将内存划分为两个区间，在任意时间点，所有动态分配的对象都只能分配在其中一个区间(称为活动区间)，而另外一个区间(称为空闲区间)则是空的。当有效内存空间耗尽时，JVM将暂停程序运行，开始复制算法GC线程。接下来GC线程会将活动区间内的存活对象，全部复制到空闲区间，且严格按照内存地址依次排列，与此同时，GC线程将更新存活对象的内存引用地址指向新的内存地址。此时，空闲区间已经与活动区间交换，而垃圾对象现在已经全部留在了原来的活动区间，也就是现在的空闲区间。事实上，在活动区间转换为空闲区间的同时，垃圾对象已经被一次性全部回收。</div>
<div>缺点：</div>
<ul>
<li>浪费了一半的内存</li>
<li>如果对象的存活率很高，假设是100%存活，那么需要将所有对象都复制一遍，并将所有的引用地址重置一遍。复制这一个工作所花费的时间，在对象存活率达到一定程度时，将会变的不可忽视。</li>
</ul>
<div>5、标记整理算法：与标记清除算法非常相似，也是分为两个阶段：标记和整理。</div>
<ul>
...
<h3><span style="font-size: 14pt;">七、说说你了解的类加载器</span></h3>
...
<h3><span style="font-size: 14pt;">八、说说你知道的JVM参数</span></h3>
<p>1、堆栈配置相关</p>
<ul>
...
<h3><span style="font-size: 14pt;">九、命令行工具</span></h3>
<ul>
<li>jps：虚拟机进程状况</li>
...
<h3><span style="font-size: 14pt;">十、调优工具</span></h3>
<ul>
...
<h3><span style="font-size: 14pt;">十一、强引用、软引用、弱引用、虚引用是什么以及应用</span></h3>
<ul>
