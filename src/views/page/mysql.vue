...
<p>原理：具体需要三个线程来操作：</p>
<ol>
<li>binlog输出线程：每当有从库连接到主库的时候，主库都会创建一个线程，然后发送binlog内容到从库。在从库里当复制开始的时候，从库就会创建两个线程进行处理。</li>
<li>从库I/O线程：当START SLAVE语句在从库开始执行之后，从库创建一个I/O线程，该线程连接到主库并请求主库发送binlog里面更新记录到从库上。从库I/O线程读取主库的binlog输出线程发送的更新并拷贝这些更新到本地文件，其中包括relay log(中继日志)文件。</li>
<li>从库的SQL线程：从库创建一个SQL线程，这个线程读取从库I/O线程写到relay log的更新事件并执行，来实现主从的操作一致，而最终数据一致。</li>
</ol>
<p>问题：</p>
<ol>
<li>主库宕机后，数据可能丢失。</li>
<li>从库只有一个sql线程，主库写压力大，复制很可能延迟。</li>
</ol>
<p>解决办法：</p>
<ol>
<li>半同步复制：解决数据库丢失的问题。</li>
...
<h4>10.3 并行复制</h4>
<p>社区版5.6中新增，并行是指从库多线程apply binlog库级别并行应用binlog，同一个库数据更改还是串行(5.7版并行复制基于事务组)设置。设置sql线程数为10：</p>
